...
Secondly, you will need an R script or RMarkdown file to use for your analysis. I have included an otherwise blank .Rmd including the setup section and shell function in the *shell_function* file in this repository, which can be downloaded and used as a template. 
...
`datadir` specifies a file path to the data you want to analyse. In most cases you will want to analyse multiple files, and so you will specify a folder rather than a single file. Using the `list.files()` function creates a list of files within the folder you specify, which should be the *Data* folder you created earlier to store all of your .bin files from the GeneActiv software. Make sure that the `full.names` argument is set to `TRUE`, so that the full file paths are listed rather than the relative file paths. Once you have run this code, check that the files are actually listed as you want (you can simply type `datadir` into the console and it should print the list of files).

`outputdir` specifies the path to the folder where you want the analysed data to be stored. In this case it is the folder called *Output*.
